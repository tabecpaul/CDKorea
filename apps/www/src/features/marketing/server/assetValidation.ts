...
export type ValidatedMarketingAsset = { filename: string; mimeType: "image/png"; byteSize: number; sha256: string; width: 1080 | 1350; height: 1350 | 1687; bytes: Uint8Array };

export class AssetValidationError extends Error {
  constructor(public code: string, public filename: string) { super(`${code}:${filename}`); }
}

export function validateMarketingPng(bytes: Uint8Array, filename: string, declaredMime = "image/png", options: { historicalRecovery?: boolean } = {}): ValidatedMarketingAsset {
  ...
  if (!((width === 1080 && height === 1350) || (options.historicalRecovery && width === 1350 && height === 1687))) throw new AssetValidationError("IMAGE_DIMENSIONS_INVALID", filename);
  return { filename, mimeType: "image/png", byteSize: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex"), width: width as ValidatedMarketingAsset["width"], height: height as ValidatedMarketingAsset["height"], bytes };
}
