...
export default function ChannelScheduleSummary({ channel, schedule }: { channel: MarketingChannel; schedule: { scheduledAt: Date; mode: string; status: string; utmUrl: string; publishedUrl: string | null; publishedAt: Date | null } }) {
  return <section className="rounded-2xl border border-navy/10 bg-white p-5 sm:p-6">
    ...
    {schedule.publishedUrl ? <div className="mt-4"><a href={schedule.publishedUrl} target="_blank" rel="noreferrer" className="inline-block text-sm font-bold text-teal underline">게시물 열기 ↗</a><p className="mt-2 text-xs text-navy/55">실제 게시 시각: {schedule.publishedAt?.toLocaleString("ko-KR", { timeZone: "Asia/Seoul" }) ?? "UNKNOWN"}</p></div> : <p className="mt-4 text-xs text-navy/45">실제 게시 증거: UNKNOWN</p>}
  </section>;
}
