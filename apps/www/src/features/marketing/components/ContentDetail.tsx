...
export default function ContentDetail({ detail }: { detail: Detail }) {
  ...
  return <>
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {[["현재 상태", currentVersion?.status ?? "버전 미등록"], ["Duplicate", currentVersion?.duplicateGate ?? "UNKNOWN"], ["Admin Approval", adminApproval ? "APPROVED" : "NOT APPROVED"], ["Site-First", currentVersion?.siteFirstStatus ?? "UNKNOWN"], ["Production Read-back", currentVersion?.canonicalReadbackStatus ?? "UNKNOWN"], ["Channel Readiness", readiness], ["현재 버전", currentVersion ? `v${currentVersion.version}` : "—"], ["CTA", detail.content.ctaKind || "미확정"]].map(([label, value]) => <article key={label} className="rounded-2xl border border-navy/10 bg-white p-5"><p className="text-xs font-bold text-navy/50">{label}</p><strong className="mt-2 block break-words">{value}</strong></article>)}
    </section>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      <div className="space-y-6">
        ...
        {!isProposal ? <ApprovalActions contentId={detail.content.id} enabled={currentVersion?.status === "review_pending"} /> : null}

        {!isProposal ? <NaverPublishingPanel contentId={detail.content.id} version={currentVersion?.version ?? null} versionStatus={currentVersion?.status ?? null} approvedSnapshotHash={currentVersion?.approvedSnapshotHash ?? null} category={detail.content.naverCategory} ctaKind={detail.content.ctaKind} naverBody={currentVersion?.naverBody ?? null} schedule={currentNaverSchedule} /> : null}

        {currentVersion && !isProposal ? <section className="grid gap-6 lg:grid-cols-3">{currentSchedules.filter((schedule) => schedule.channel !== "naver").map((schedule) => <ManualPublicationPanel key={schedule.id} contentId={detail.content.id} channel={schedule.channel as "facebook" | "instagram" | "threads"} versionStatus={currentVersion.status} approvedSnapshotHash={currentVersion.approvedSnapshotHash} schedule={schedule} />)}</section> : null}
